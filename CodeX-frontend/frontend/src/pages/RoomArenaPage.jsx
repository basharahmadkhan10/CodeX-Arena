    ...
    <div className="w-64 shrink-0 border-r-2 border-black bg-[#f8f8f8] flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        {sortedParticipants.map((p, idx) => {
          ...
          return (
            <div key={p.userId} className={`rounded-xl border-2 border-black p-2.5 shadow-[2px_2px_0px_#000] ${isMe ? "bg-white" : "bg-[#f0fafa]"}`}>
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-1.5 min-w-0">
                  <span className="text-xs font-black text-black/40 w-5">#{idx + 1}</span>
                  <div className={`w-6 h-6 rounded-lg border-2 border-black flex items-center justify-center text-[10px] font-black shrink-0 ${isMe ? "bg-[rgb(238,11,22)] text-white" : "bg-white"}`}>
                    {p.username[0].toUpperCase()}
                  </div>
                  <span className="text-xs font-black text-black truncate">{isMe ? "You" : p.username}</span>
                </div>
                <div className="flex items-center gap-1">
                  <span className="text-sm font-black text-[rgb(238,11,22)]">{score}</span>
                  <span className="text-[9px] font-black text-black/40">/{totalQuestions}</span>
                  {disconnected && <WifiOff size={11} className="text-red-400 shrink-0" />}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-1">
                {currentSubmitting ? (
                  <div className="flex items-center gap-1 bg-amber-50 border border-amber-300 rounded-lg px-2 py-1 flex-1">
                    ...
                    <span className="text-[9px] font-black text-amber-600">Judging...</span>
                  </div>
                ) : status === "AC" ? (
                  <div className="flex items-center gap-1 bg-green-100 border border-green-400 rounded-lg px-2 py-1 flex-1">
                    <CheckCircle size={10} className="text-green-600" />
                    <span className="text-[9px] font-black text-green-700">Solved!</span>
                  </div>
                ) : status && status !== "pending" ? (
                  <div className="flex items-center justify-between bg-[#f0fafa] border border-black/10 rounded-lg px-2 py-1 flex-1">
                    <span className={`text-[9px] font-black ${status === "WA" ? "text-red-500" : status === "RE" || status === "CE" ? "text-orange-500" : "text-black/50"}`}>
                      {status}
                    </span>
                    {live.passed !== undefined && live.total !== undefined && (
                      <span className="text-[9px] font-black text-black/50">{live.passed}/{live.total}</span>
                    )}
                  </div>
                ) : (
                  <div className="bg-[#e8e8e8] border border-black/10 rounded-lg px-2 py-1 flex-1">
                    <span className="text-[9px] font-bold text-black/30">Working...</span>
                  </div>
                )}
              </div>
            </div>
...
function RoomResultModal({ result, myUserId, onClose }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <motion.div initial={{ scale: 0.88, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.88, y: 30 }} className="bg-white border-2 border-black shadow-[10px_10px_0px_#000] rounded-2xl p-7 w-full max-w-md">
        <div className={`text-center mb-6 p-5 rounded-xl border-2 border-black shadow-[3px_3px_0px_#000] ${isDraw ? "bg-gray-100" : isWinner ? "bg-green-100" : "bg-red-100"}`}>
          <div className="text-4xl mb-2">{isDraw ? "🤝" : isWinner ? "🏆" : "💀"}</div>
          <h2 className={`text-2xl font-black ${isDraw ? "text-gray-700" : isWinner ? "text-green-700" : "text-red-600"}`}>
            {isDraw ? "Draw!" : isWinner ? "You Won!" : "You Lost!"}
          </h2>
          <p className="text-xs font-black text-black/40 uppercase tracking-widest mt-1">
            {reason === "solved" ? "First to solve all questions" : reason === "timeout" ? "Time's up" : reason === "forfeit" ? "Forfeit" : "Disconnect"}
        ...
        <div className="space-y-2 mb-6 max-h-96 overflow-y-auto">
          {sorted.map((p, i) => {
            const isMe = p.userId === myUserId;
            return (
              <div key={p.userId} className={`flex items-center justify-between rounded-xl border-2 border-black px-3 py-2.5 shadow-[2px_2px_0px_#000] ${isMe ? "bg-[rgb(238,11,22)]/10" : "bg-[#f8f8f8]"}`}>
                <div className="flex items-center gap-2.5">
                  <span className="text-sm font-black text-black/40 w-4">{i + 1}</span>
                  <span className="text-sm font-black text-black">{isMe ? "You" : p.username}{p.userId === winnerId && " 🏆"}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-black text-[rgb(238,11,22)]">{p.solvedCount}/{totalQuestions}</span>
                  <span className={`text-xs font-black px-2 py-0.5 rounded-lg border border-black ${p.ratingChange >= 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
        ...
        <button onClick={onClose} className="w-full py-3 bg-black text-white font-black text-sm uppercase tracking-widest border-2 border-black rounded-xl shadow-[3px_3px_0px_#000] hover:shadow-[1px_1px_0px_#000] hover:translate-x-[2px] hover:translate-y-[2px] transition-all">
          Back to Lobby
        </button>
      </motion.div>
    </motion.div>
        ...
        {questions.map((q, idx) => (
          <button
            ...
            className={`relative shrink-0 w-10 h-10 rounded-xl border-2 border-black font-black text-sm transition-all shadow-[2px_2px_0px_#000] ${
              currentIndex === idx ? "bg-[rgb(238,11,22)] text-white scale-105" : solvedStatus[idx] ? "bg-green-100 text-green-700" : "bg-white text-black hover:bg-gray-100"
            }`}
          >
            {idx + 1}
            {solvedStatus[idx] && <CheckCircle size={12} className="absolute -top-1 -right-1 text-green-600 bg-white rounded-full" />}
          </button>
        ))}
      </div>
    </div>
...
export default function RoomArenaPage() {
  ...
  // Manual fullscreen request function
  ...
  const autoForfeit = useCallback(
    (reason) => {
      ...
      setTimeout(() => {
        ...
        if (socket) {
          socket.emit("room:forfeit", { battleId: roomBattle.battleId, roomId: roomBattle.roomId });
        }
      }, 1000);
    },
  ...
  // Fullscreen detection - FIXED: no auto-request
  useEffect(() => {
    if (!isBattleActive) return;
    const handleFullscreenChange = () => {
      const isCurrentlyFullscreen = !!document.fullscreenElement;
      setIsFullscreen(isCurrentlyFullscreen);
      if (!isCurrentlyFullscreen && isFullscreen && !hasAutoForfeited) {
    ...
    const handleVisibilityChange = () => {
      ...
      if (!isVisible && isBattleActive) {
        setFocusLostCount((prev) => {
          ...
          if (remaining > 0) {
            toast.error(`⚠️ Tab switching detected! ${remaining} warning${remaining === 1 ? "" : "s"} remaining.`, { duration: 3000 });
          }
          return newCount;
        });
  ...
  // Periodic checks - FIXED: no auto fullscreen request
  useEffect(() => {
    if (!isBattleActive) return;
    violationIntervalRef.current = setInterval(() => {
      ...
      if (!document.fullscreenElement && isBattleActive && !hasAutoForfeited && isFullscreen) {
  ...
  // Keyboard shortcut blocking - FIXED: no toast.warning
  useEffect(() => {
    if (!isBattleActive) return;
    ...
    const handleKeyDown = (e) => {
      if (e.key === "Escape" && isFullscreen && isBattleActive) {
        e.preventDefault();
        toast.error("⚠️ Escape exits fullscreen and counts as violation!", { duration: 2000 });
        setFullscreenExits((prev) => prev + 1);
        setViolations((prev) => prev + 2);
        return false;
      }
  ...
  useEffect(() => {
    ...
    socket.on("room:submission_result", (result) => {
      ...
      if (result.status === "AC") {
        ...
        if (currentQuestionIndex + 1 < totalQuestions) {
          setTimeout(() => setCurrentQuestionIndex(currentQuestionIndex + 1), 1500);
        }
    ...
    socket.on("room:submission_update", ({ userId, status, passed, total, solvedCount }) => {
      setLiveStatuses((prev) => ({
        ...prev,
        [userId]: { ...prev[userId], status, passed, total, isSubmitting: false, solvedCount: solvedCount || Object.values(solvedQuestions).filter(Boolean).length },
      }));
    });
    socket.on("room:opponent_submitting", ({ userId }) => {
      setLiveStatuses((prev) => ({ ...prev, [userId]: { ...prev[userId], isSubmitting: true } }));
    });
    ...
    socket.on("room:opponent_disconnected", ({ userId }) => {
      setLiveStatuses((prev) => ({ ...prev, [userId]: { ...prev[userId], disconnected: true } }));
  ...
  const handleSubmit = () => {
    ...
    if (isCurrentSolved) { toast.error("You already solved this question!"); return; }
    ...
    setSubmissionResult(null);
    socket.emit("room:submit", { battleId, code, language, questionIndex: currentQuestionIndex });
  ...
  const handleQuestionChange = (index) => {
    if (index !== currentQuestionIndex) setCurrentQuestionIndex(index);
  };

  const handleEditorMount = (editor, monaco) => {
    editorRef.current = editor;
    editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyV, () => {
      toast.error("Paste is disabled.", { duration: 2000 });
      setViolations((prev) => prev + 1);
    });
    editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyC, () => {
      toast.error("Copy is disabled.", { duration: 2000 });
      setViolations((prev) => prev + 0.5);
    });
    editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyX, () => {
      toast.error("Cut is disabled.", { duration: 2000 });
      setViolations((prev) => prev + 0.5);
    });
  ...
  return (
    <div className="h-screen bg-[rgb(238,11,22)] flex flex-col overflow-hidden">
      {/* Fullscreen warning overlay - FIXED: uses manual function */}
      <AnimatePresence>
        {!isFullscreen && isBattleActive && !hasAutoForfeited && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/90 flex items-center justify-center z-50">
            <div className="bg-white border-4 border-black rounded-2xl p-8 text-center max-w-md mx-4 shadow-[12px_12px_0px_#000]">
              ...
              <button onClick={requestFullscreenManually} className="bg-[rgb(238,11,22)] text-white font-black px-6 py-3 rounded-xl border-2 border-black shadow-[4px_4px_0px_#000] transition-all">
                Enter Fullscreen
              </button>
            </div>
          </motion.div>
      ...
      <header className="h-13 bg-white border-b-2 border-black flex items-center px-4 gap-4 shrink-0 z-10 py-2">
        ...
        <div className="flex items-center gap-2 bg-[#f0fafa] border-2 border-black rounded-xl px-3 py-1 shadow-[2px_2px_0px_#000]">
          ...
          <span className="text-xs font-black text-black">{solvedCount}/{totalQuestions} Solved</span>
        </div>
        ...
        {(violations > 0 || fullscreenExits > 0 || focusLostCount > 0) && (
          <div className="flex flex-col items-end gap-0.5">
            ...
            <div className="flex gap-2 text-[10px] font-black">
              {fullscreenExits > 0 && <span className="px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">🖥️ {remainingFullscreenExits}/{MAX_FULLSCREEN_EXITS}</span>}
              {focusLostCount > 0 && <span className="px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">🔄 {remainingTabSwitches}/{MAX_TAB_SWITCHES}</span>}
            </div>
          </div>
        )}
      ...
      <div className="flex-1 flex overflow-hidden">
        <PlayersPanel participants={participants} liveStatuses={liveStatuses} myUserId={myUserId} roomCode={roomCode} currentQuestionIndex={currentQuestionIndex} totalQuestions={totalQuestions} />
        ...
        <div className="w-[420px] shrink-0 border-r-2 border-black flex flex-col overflow-hidden bg-white">
          <div className="flex border-b-2 border-black shrink-0 bg-[#f0fafa]">
            {TABS.map((t) => (
              <button key={t} onClick={() => setActiveTab(t)} className={`flex-1 py-2.5 text-xs font-black uppercase tracking-widest transition-all border-r-2 border-black last:border-r-0 ${activeTab === t ? "bg-[rgb(238,11,22)] text-white" : "text-black/40 hover:text-black hover:bg-[#e0e8e8]"}`}>
                {t}
                {t === "results" && submissionResult && <span className={`ml-1.5 inline-block w-2 h-2 rounded-full border border-black ${submissionResult.status === "AC" ? "bg-green-400" : "bg-red-400"}`} />}
              </button>
            ))}
          </div>
            ...
            {activeTab === "problem" && currentProblem && (
              <div className="p-5 space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <h2 className="text-lg font-black text-black leading-tight">{currentQuestionIndex + 1}. {currentProblem.title}</h2>
                  <DiffBadge diff={currentProblem.difficulty} />
                </div>
                {currentProblem.tags?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {currentProblem.tags.map((t) => <span key={t} className="text-xs bg-[#e0f8f8] text-black font-bold px-2 py-0.5 rounded-lg border-2 border-black/20">{t}</span>)}
                  </div>
                )}
                <div className="text-sm text-black/70 leading-relaxed whitespace-pre-wrap font-medium">{currentProblem.description}</div>
                {currentProblem.constraints && (
                  <div className="bg-[#f0fafa] border-2 border-black rounded-xl p-3 shadow-[2px_2px_0px_#000]">
                    ...
                    <p className="text-xs text-black/60 font-mono whitespace-pre-wrap">{currentProblem.constraints}</p>
                  </div>
                )}
                {currentProblem.examples?.map((ex, i) => (
                  <div key={i} className="border-2 border-black rounded-xl overflow-hidden shadow-[3px_3px_0px_#000]">
                    ...
                    <div className="p-3 space-y-2 bg-white">
                      <div><p className="text-xs text-black/40 font-bold mb-1">Input:</p><pre className="text-xs text-green-700 font-mono bg-green-50 border border-green-200 rounded-lg p-2 whitespace-pre-wrap">{ex.input}</pre></div>
                      <div><p className="text-xs text-black/40 font-bold mb-1">Output:</p><pre className="text-xs text-blue-700 font-mono bg-blue-50 border border-blue-200 rounded-lg p-2 whitespace-pre-wrap">{ex.output}</pre></div>
                ...
                <div className="text-xs text-black/40 font-bold flex items-center gap-3 pt-1">
                  <span>{currentProblem.sampleTestCases?.length || 0} sample tests visible</span>
                  <span>·</span>
                      ...
                      {submissionResult.results?.map((r, i) => (
                        <div key={i} className={`rounded-xl border-2 border-black p-3 shadow-[2px_2px_0px_#000] ${r.passed ? "bg-green-50" : "bg-red-50"}`}>
                          <div className="flex items-center justify-between mb-1.5">
                            ...
                            <span className={`text-xs font-black px-2 py-0.5 rounded-lg border border-black ${r.passed ? "bg-green-200 text-green-700" : "bg-red-200 text-red-600"}`}>{r.passed ? "PASS ✓" : "FAIL ✗"}</span>
                          </div>
                          {r.isPublic && (
            ...
            {activeTab === "output" && (
              <div className="p-4 space-y-3">
                <div>
                  ...
                  <textarea value={customInput} onChange={(e) => setCustomInput(e.target.value)} placeholder="Enter input here..." rows={4} className="w-full bg-[#f0fafa] border-2 border-black rounded-xl p-3 text-sm text-black font-mono placeholder-black/25 focus:outline-none focus:border-[rgb(238,11,22)] resize-none shadow-[2px_2px_0px_#000]" />
                </div>
                {runResult && (
                  <div>
                    <label className="text-xs font-black text-black uppercase tracking-widest mb-1.5 block">Output</label>
                    <pre className={`border-2 border-black rounded-xl p-3 text-xs font-mono whitespace-pre-wrap shadow-[2px_2px_0px_#000] ${runResult.error ? "bg-red-50 text-red-600" : "bg-[#f0fafa] text-green-700"}`}>{runResult.output || "(no output)"}</pre>
                  </div>
                )}
              </div>
            )}
          ...
          <QuestionNavigator questions={roomBattle.questions} currentIndex={currentQuestionIndex} onSelect={handleQuestionChange} solvedStatus={solvedQuestions} />
        </div>

        {/* RIGHT: Code Editor */}
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="h-11 border-b-2 border-black bg-[#f0fafa] flex items-center px-4 gap-3 shrink-0">
            <select value={language} onChange={(e) => setLanguage(e.target.value)} className="bg-white border-2 border-black text-black text-sm rounded-lg px-3 py-1.5 focus:outline-none cursor-pointer font-bold shadow-[2px_2px_0px_#000]">
              {LANGUAGE_OPTIONS.map((l) => <option key={l.value} value={l.value}>{l.label}</option>)}
            </select>
            <div className="flex-1" />
            <button onClick={handleRun} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border-2 border-black text-black font-black text-xs uppercase tracking-wider hover:bg-[rgb(238,11,22)] hover:text-white transition-all shadow-[2px_2px_0px_#000]">
              <Play size={13} /> Run
            </button>
            <button onClick={handleSubmit} disabled={isSubmitting || isCurrentSolved} className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-[rgb(238,11,22)] hover:bg-[rgb(218,8,20)] disabled:opacity-50 disabled:cursor-not-allowed text-white font-black text-xs uppercase tracking-wider border-2 border-black shadow-[3px_3px_0px_#000] hover:shadow-[1px_1px_0px_#000] transition-all">
              {isSubmitting ? <><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Judging...</> : isCurrentSolved ? <><CheckCircle size={13} /> Solved!</> : <><Send size={13} /> Submit</>}
            </button>
          </div>

          <div className="flex-1 overflow-hidden">
            <Editor height="100%" language={language} value={code} onChange={(val) => setCode(val || "")} onMount={handleEditorMount} theme="vs-dark" options={{
              fontSize: 14, fontFamily: "JetBrains Mono, Fira Code, monospace", minimap: { enabled: false }, scrollBeyondLastLine: false,
              padding: { top: 16, bottom: 16 }, lineNumbers: "on", wordWrap: "on", tabSize: 2, automaticLayout: true, contextmenu: false,
              suggestOnTriggerCharacters: false, quickSuggestions: false, parameterHints: { enabled: false }, hover: { enabled: false },
            }} />
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {battleResult && <RoomResultModal result={battleResult} myUserId={myUserId} onClose={handleClose} />}
      </AnimatePresence>
    </div>
  );
}
